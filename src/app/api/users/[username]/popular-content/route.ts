...
export async function GET(
  ...
  try {
    ...
    // Get popular reviews (by like count) with actual ratings
    const popularReviews = await prisma.$transaction(async (tx) => {
    ...
    // Get popular discussions (by like count)
    ...
    const formattedDiscussions = popularDiscussions.map(discussion => {
      let entityName = '';
      let entityId = '';
      let entityType = '';
    ...
    // Get popular watchlists (by like count) with privacy filtering
    const popularWatchlists = await prisma.watchList.findMany({
      where: { 
        userId: targetUserId,
        OR: [
          { isPublic: true },
    ...
    const formattedWatchlists = popularWatchlists.map(watchlist => ({
