...
export default function AddToWatchListModal({
            ...
            const fetchWatchLists = async () => {
                try {
                    const response = await fetch(
                        `/api/users/${user?.username}/watch-lists`
                    );
                    if (response.ok) {
        ...
        const searchWatchLists = async (query: string) => {
            ...
            try {
                const response = await fetch(
                    `/api/users/${
                        user?.username
                    }/watch-lists/search?q=${encodeURIComponent(query)}`
                );
                if (response.ok) {
                    const data = await response.json();
                    setWatchLists(data.watchLists || []);
    ...
    const handleAddToWatchList = async (watchListId: number) => {
        ...
        try {
            const response = await fetch(
                `/api/watch-lists/${watchListId}/shows`,
                {
                    method: "POST",
                    headers: {
            ...
            if (response.ok) {
                ...
                if (
                    response.status === 400 &&
                    errorData.error === "Show already in watch list"
                ) {
                    const watchList = watchLists.find(
                        (wl) => wl.id === watchListId
                    );
                    setErrorMessage(
                        `Show already on '${watchList?.name}' watch list`
                    );
                } else {
    ...
    return (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm md:bg-white/5 flex items-center justify-center z-50 p-2 sm:p-4">
            <div className="bg-gray-800 rounded-lg max-w-md w-full max-h-[90vh] sm:max-h-[80vh] overflow-hidden">
                {/* Header */}
                    ...
                    <div className="mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                ...
                                className="flex-grow px-3 py-2 bg-gray-600 border border-gray-500 rounded-md text-white focus:outline-none focus:border-green-400 text-base min-w-0"
                            />
                        </div>
                    </div>
                            ...
                            watchLists.map((watchList) => (
                                ...
                                >
                                    <div className="flex items-start justify-between min-w-0">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {watchList.description && (
                                                <p className="text-gray-300 text-sm break-words mt-1">
                                                    {watchList.description.length > 200 
                                                        ? `${watchList.description.substring(0, 200)}...`
                                                        : watchList.description
                                                    }
                                                </p>
                                            )}
                                        ...
                                        <span className="text-gray-400 text-xs flex-shrink-0 ml-2 mt-1">
